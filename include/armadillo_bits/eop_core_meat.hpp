...
eop_core<eop_type>::apply(outT& out, const eOp<T1, eop_type>& x)
  ...
  if(Proxy<T1>::use_at == false)
    ...
    if(use_mp && mp::meets_thresh<(is_cx<eT>::yes)>(n_elem))
      {
    ...
    if(use_mp && mp::meets_thresh<(is_cx<eT>::yes)>(x.get_n_elem()))
      {
      arma_applier_2_mp(=);
      }
    else
...
eop_core<eop_type>::apply(Cube<typename T1::elem_type>& out, const eOpCube<T1, eop_type>& x)
  ...
  if(ProxyCube<T1>::use_at == false)
    ...
    if(use_mp && mp::meets_thresh<(is_cx<eT>::yes)>(x.get_n_elem_slice()))
      {
      arma_applier_3_mp(=);
      }
    else
